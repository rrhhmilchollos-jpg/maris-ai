...
const jobId = `spax-v16-${randomUUID()}`;
...
async function main() {
  ...
  if (!validation.ok) throw new Error(`Bundle v16 rejected: ${JSON.stringify(validation.issues)}`);
  ...
  try {
    await insertAppRevisionFromRow({ row: locked.lease.app, source: "edit", summary: "Snapshot previo a SPAX v16: acceso común por código numérico de empleado y contraseña.", jobId });
    const committed = await commitAppMutation({
      ...
      update: {
        frontendCode,
        status: "ready",
        evaluatorSummary: "SPAX v16 validada: acceso por código de empleado de seis dígitos y contraseña para moderación, veterinaria, voluntariado y administración.",
      },
    });
    if (!committed.ok) throw new Error("SPAX mutation conflict: no changes persisted");
    await AppMessage.create({ appId, role: "assistant", content: "SPAX v16 aplicada con snapshot. El acceso profesional ahora solicita código numérico de empleado y contraseña para todos los roles; el rol se confirma únicamente desde el servidor." });
