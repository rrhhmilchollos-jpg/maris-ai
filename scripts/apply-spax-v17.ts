...
const jobId = `spax-v17-${randomUUID()}`;
...
async function main() {
  ...
  if (!validation.ok) throw new Error(`Bundle v17 rejected: ${JSON.stringify(validation.issues)}`);
  ...
  try {
    await insertAppRevisionFromRow({ row: locked.lease.app, source: "edit", summary: "Snapshot previo a SPAX v17: perfiles completos de perros y acciones desde fotografía.", jobId });
    const committed = await commitAppMutation({
      ...
      update: {
        frontendCode,
        status: "ready",
        evaluatorSummary: "SPAX v17 validada: fotografías accesibles, fichas completas de perros y solicitudes de adopción o acogida desde cada perfil.",
      },
    });
    if (!committed.ok) throw new Error("SPAX mutation conflict: no changes persisted");
    await AppMessage.create({ appId, role: "assistant", content: "SPAX v17 aplicada con snapshot. Cada foto abre una ficha completa con carácter, hogar, convivencia, paseos, cuidados y acciones de adopción o acogida." });
