 ...
 *   - Frontend Eng  (claude-4-7-sonnet)  — bundle frontend
 *   - Backend Eng   (claude-4-7-sonnet)  — bundle backend
...
async function architectPlan(prompt: string, research: string): Promise<ProjectPlan> {
  ...
  if (!raw) {
    // Fallback: retry with claude-4-7-sonnet (same model, fresh attempt)
    try {
      const response = await withTimeoutOrThrow(
        anthropic.messages.create({
          model: "claude-haiku-4-5",
