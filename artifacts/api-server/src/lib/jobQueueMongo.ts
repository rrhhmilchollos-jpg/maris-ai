// jobQueueMongo.ts — implementación de respaldo (single-instance, sin Redis).
// A petición explícita del usuario ("lo de redis hay que hacerlo sí o sí"):
// este archivo era antes "jobQueue.ts" -- ahora ese nombre lo tiene un router
// (ver jobQueue.ts) que elige automáticamente entre esta implementación y
// jobQueue-bullmq.ts (Redis, escalable, multi-instancia) según si
// REDIS_URL está configurada. Contenido sin cambios respecto al original,
// solo renombrado -- nada de la lógica interna se ha tocado.
...
// puede absorber la cuenta de Zoco IA sin entrar en rate limiting 429
...
// hasta 25 (antes 10) para quien tenga un tier de Zoco IA con más
