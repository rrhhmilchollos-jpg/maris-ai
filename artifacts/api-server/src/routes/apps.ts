  ...
  if (previous) {
    ...
    if (directLiteralEdit) {
      ...
      try {
        ...
        const editChangedBundle = editResult.frontendCode !== previous.frontendCode || (editResult.backendCode || previous.backendCode) !== previous.backendCode;
        if (editResult.frontendCode && editResult.frontendCode.trim().length > 0 && editChangedBundle) {
          result = {
            title: previous.title,
            description: previous.description,
            techStack: previous.techStack,
          ...
          await log("system", "El orquestador de edición no produjo un cambio verificable; continúo con el pipeline robusto de una sola pasada como respaldo sin marcar una edición vacía como éxito.", "warn");
        }
...
// ── POST /api/apps ────────────────────────────────────────────────────────
// Los endpoints clerk-user-count y clerk-sync-users se han retirado junto
