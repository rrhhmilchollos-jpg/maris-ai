...
import {
  ...
  Paperclip, Send, Sparkles, Github, Search, Mic, Square,
                ...
                <div className="flex items-center gap-1">
                  ...
                  {isSpeechSupported && (
                    <button
                      onClick={toggleMic}
                      disabled={generateAppMutation.isPending || (isActive && !isDone)}
                      className={`p-1.5 rounded-lg transition-all relative disabled:opacity-40 ${
                        isListening
                          ? "text-red-400 bg-red-500/10 hover:bg-red-500/20"
                          : "text-white/30 hover:text-violet-400 hover:bg-violet-500/10"
                      }`}
                      title={isListening ? "Detener grabación" : "Hablar por voz (es-ES)"}
                    >
                      {isListening && (
                  ...
                  <div className="text-[10px] text-white/20 ml-1">
                    {isListening
                      ? <><span className="inline-block h-1.5 w-1.5 rounded-full bg-red-400 animate-pulse mr-1" />Escuchando...</>
                      : isActive
                        ? <><Clock className="h-3 w-3 inline mr-1" />Generando...</>
                        : "Enter para enviar"
                    }
                  </div>
                </div>
